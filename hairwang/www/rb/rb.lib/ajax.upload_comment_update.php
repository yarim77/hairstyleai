...
if($files){
  ...
  $row = sql_fetch(" select max(bf_no) as num from {$g5['board_file_table']} where bo_table='{$bo_table}' and wr_id ='{$comment_id}'");
...
if($del){
  sql_query('delete from `'.$g5['board_file_table'].'` where bo_table="'.$bo_table.'" and wr_id='.$comment_id.' and bf_file in ("'.implode('","',array_values($del)).'")');
}
$sql = "select * from {$g5['board_file_table']} where bo_table='{$bo_table}' and wr_id='{$comment_id}'";
$result = sql_query($sql);
$i=0;
while($row = sql_fetch_array($result)){
  $sql = "update {$g5['board_file_table']} set bf_no='{$i}' where bo_table='{$bo_table}' and wr_id='{$comment_id}' and bf_no='{$row['bf_no']}'";
  sql_query($sql);
  $i++;
}
...
$rows = sql_fetch(" select count(*) as cnt from {$g5['board_file_table']} where bo_table = '{$bo_table}' and wr_id = '{$comment_id}' ");
sql_query(" update {$write_table} set wr_file = '{$rows['cnt']}' where wr_id = '{$comment_id}' ");
?>
