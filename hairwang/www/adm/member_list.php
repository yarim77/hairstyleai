...
/* proc_mode 변수 정의 추가 */
$proc_mode = isset($_REQUEST['proc_mode']) ? $_REQUEST['proc_mode'] : '';
$login_us_id = isset($_REQUEST['login_us_id']) ? $_REQUEST['login_us_id'] : '';
...
if($proc_mode=='login'){
	...
	if($mb){
		...
		alert($mb['mb_id']." 아이디로 로그인합니다.", G5_URL);
	}
	else {
		alert('존재하지 않는 회원입니다.');
	}
...
// 레벨 선택 함수 재정의
function get_member_level_select_custom($name, $start_id=0, $end_id=10, $selected="", $event="", $level_names=array())
{
    global $g5;
    ...
    for ($i=$start_id; $i<=$end_id; $i++) {
        ...
        $level_title = isset($level_names[$i]) ? $level_names[$i] : '';
        $str .= ">Lv.{$i} {$level_title}</option>\n";
    }
    $str .= "</select>\n";
    return $str;
}
                ...
                ?>
                    ...
                    <tr class="<?php echo $bg; ?>">
                        ...
                        <td headers="mb_list_id" colspan="2" class="td_name sv_use">
                            <a href="<?php echo $_SERVER['SCRIPT_NAME']; ?>?proc_mode=login&login_us_id=<?php echo  $mb_id; ?>"><?php echo $mb_id; ?></a>
                            <?php
                            //소셜계정이 있다면
						...
						<td headers="mb_list_mng" rowspan="2" class="td_mng td_mng_s"><?php echo $s_mod ?><?php echo $s_grp ?><a href="./member_force_delete.php?mb_id=<?php echo $row['mb_id'] ?>" onclick="return confirm('정말 이 회원을 강제삭제 하시겠습니까?');" class="btn btn_02">강제삭제</a>
						</td>
                    </tr>
                    <tr class="<?php echo $bg; ?>">
                        ...
                        <td headers="mb_list_auth" class="td_mbstat">
                            <?php echo get_member_level_select_custom("mb_level[$i]", 1, $member['mb_level'], $row['mb_level'], "", $level_names) ?>
                        </td>
