...
// 회원권한을 SELECT 형식으로 얻음, 숫자가 아닌 명칭을 사용_2025_01_03 by jay
function get_member_level_select($name, $start_level = 1, $end_level = 10, $selected_level = 1)
{
    ...
    for ($i = $start_level; $i <= $end_level; $i++) {
        ...
        if ($i == $selected_level)
            $str .= ' selected="selected"';
        $str .= '>' . $member_levels[$i] . '</option>'; // config.php에서 불러온 함수
    }
