...
<ul class="btn_top top btn_bo_user"> 
	...
	<li>
	    ...
	    <ul class="more_opt is_view_btn">
			...
			// 잠금 글이면 신고 버튼 숨김 
			if (!$is_locked_post) { ?><li><a href="javascript:void(0);" onclick="handleReportClick('<?php echo $bo_table; ?>', '<?php echo $view['wr_id']; ?>')"><i class="fa fa-flag"></i> 신고</a></li><?php }
			//////////////////////////////////////////////////
			//// 2. 신고 게시글 끝 //////////////////////////////
			//////////////////////////////////////////////////
			?>
		...
		*/
		...
		?>
			<div class="report-admin-lock-banner">
				...
				<a href="<?php echo $admin_report_url; ?>" target="_blank">
					신고 내역 보기
		...
		?>
			<div id="bo_v_con">
				<div class="report-admin-lock-banner">
					...
					신고가 누적되어 본문이 비공개 처리되었습니다.<br>관리자의 확인 후 복구될 수 있습니다.
				</div>
			</div>
		<?php } else { ?>
			<div id="bo_v_con"><?php echo get_view_thumbnail($view['content']); ?></div>
...
function hideCommentContent(comment_id) {
  ...
  if (!p) return;
  p.innerHTML = '<div class="cmt-report-hidden-note">이 콘텐츠는 신고 또는 차단으로 인해 일시적으로 비공개 처리되었습니다. 신고된 콘텐츠는 24시간 내 검토됩니다.</div>';
}
function hidePostContent() {
  const con = document.getElementById('bo_v_con');
  if (!con) return;
  con.innerHTML = '<div class="report-post-hidden-note">이 콘텐츠는 신고 또는 차단으로 인해 일시적으로 비공개 처리되었습니다. 신고된 콘텐츠는 24시간 내 검토됩니다.</div>';
}
