...
if($_POST['js'] == "on") {
    ...
    {
        if($write['mb_id'] == $member['mb_id']) {	
            $error = '자신의 댓글에는 좋아요 하실 수 없습니다.';
            print_result($error, $count);
        }

        $sql = " select bg_flag from {$g5['reple_good_table']}
                    where bo_table = '{$bo_table}'
                    and wr_id = '{$wr_id}'
                    and mb_id = '{$member['mb_id']}'
                    and bg_flag in ('good', 'nogood') ";
        ...
        else
        {
            ...
            sql_query(" update {$g5['write_prefix']}{$bo_table} set wr_{$good} = wr_{$good} + 1 where wr_id = '{$wr_id}' ");
            // 내역 생성
            sql_query(" insert {$g5['reple_good_table']} set bo_table = '{$bo_table}', wr_id = '{$wr_id}', mb_id = '{$member['mb_id']}', bg_flag = '{$good}', bg_datetime = '".G5_TIME_YMDHIS."' ");
