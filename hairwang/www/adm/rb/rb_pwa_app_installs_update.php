<?php
// /adm/rb/rb_pwa_app_installs_update.php
...
if ($act === '선택삭제') {
    ...
    if ($ids) {
        $in = implode(',', $ids);
        sql_query("DELETE FROM rb_pwa_app_installs WHERE id IN ({$in})");
    }
    goto_url('./rb_pwa_app_installs.php?'.$qstr.'&page='.$page); exit;

} else if ($act === '전체삭제') {
    sql_query("TRUNCATE TABLE rb_pwa_app_installs");
    goto_url('./rb_pwa_app_installs.php'); exit;
}

alert('잘못된 요청입니다.', './rb_pwa_app_installs.php');
