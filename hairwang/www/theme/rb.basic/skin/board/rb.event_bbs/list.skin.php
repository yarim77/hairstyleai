...
// 이벤트 필터링을 위한 파라미터 처리
$event_status = isset($_GET['event_status']) ? $_GET['event_status'] : '';
$rb_color = str_replace('co_', '#', $rb_core['color']);
?>
<style>
    .event_filter_buttons .filter_btn.active {  background: <?php echo $rb_color ?>; color: #fff; border-color: <?php echo $rb_color ?>;}
    .event_filter_buttons .filter_btn.active:hover {background: <?php echo $rb_color ?>;}
</style>
<!--
...
<div class="rb_bbs_wrap" id="scroll_container" style="width:<?php echo $width; ?>">
    ...
    <!-- 이벤트 필터 버튼 { -->
    <div class="event_filter_buttons">
        <a href="javascript:void(0);" class="filter_btn active" data-status="all">전체</a>
        <a href="javascript:void(0);" class="filter_btn" data-status="ongoing">진행중</a>
        <a href="javascript:void(0);" class="filter_btn" data-status="upcoming">준비중</a>
        <a href="javascript:void(0);" class="filter_btn" data-status="ended">종료됨</a>
    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.filter_btn').click(function() {
                ...
                // 모든 이벤트 항목 숨기기
                $('.swiper-slide-<?php echo $bo_table ?>').hide();
                
                if (status === 'all') {
                    // 전체 보기일 경우 모든 항목 표시
                    $('.swiper-slide-<?php echo $bo_table ?>').show();
                } else {
                    // 해당 상태의 항목만 표시
                    $('.swiper-slide-<?php echo $bo_table ?>[data-event-status="' + status + '"]').show();
                }
                
                // Swiper 업데이트 (레이아웃 재조정)
                swiper.update();
            });
        });
    </script>
    ...
    <ul class="gallery_top_mt rb_bbs_list swiper-wrapper swiper-wrapper-<?php echo $bo_table ?>">
        ...
        <div class="gallery_v_mtop swiper-slide swiper-slide-<?php echo $bo_table ?>" <?php echo $rb_ev_stl ?> data-event-status="<?php echo $event_type; ?>">
            ...
            <?php if (isset($list[$i]['wr_3']) && $list[$i]['wr_3']) { ?>
                <a href="<?php echo $wr_href ?>"><?php echo $list[$i]['wr_3'] ?></a>
            <?php } ?>
            </ul>
