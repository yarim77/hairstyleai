...
function get_sideview($mb_id, $name='', $email='', $homepage='')
{
    ...
    if ($mb_id) {
        ...
        if ($config['cf_use_member_icon']) {
            ...
            // 회원이미지 파일 우선 체크 (다양한 확장자 지원)
            $member_image_extensions = array('gif', 'jpg', 'jpeg', 'png', 'webp');
            $member_image_found = false;
            $member_image_url = '';
            
            foreach ($member_image_extensions as $ext) {
                $member_image_file = G5_DATA_PATH.'/member/'.$mb_id.'.'.$ext;
                if (file_exists($member_image_file)) {
                    $member_image_url = G5_DATA_URL.'/member/'.$mb_id.'.'.$ext;
                    $member_image_found = true;
                    break;
                }
            }
            
            // 회원이미지가 있으면 회원이미지를 아이콘으로 사용
            if ($member_image_found) {
                $width = $config['cf_member_icon_width'];
                $height = $config['cf_member_icon_height'];
                $name_tag['profile_image'] = '<span class="profile_img member_image_icon">';
                $name_tag['profile_image'] .= '<img src="'.$member_image_url.'" width="'.$width.'" height="'.$height.'" alt="'.$name.'" style="border-radius: 50%; object-fit: cover;">';
                $name_tag['profile_image'] .= '</span>';
                ...
                // 회원이미지가 없으면 기존 회원아이콘 체크
                $icon_file = G5_DATA_PATH.'/member_image/'.$mb_dir.'/'.get_mb_icon_name($mb_id).'.gif';
                
                if (file_exists($icon_file)) {
                    $width = $config['cf_member_icon_width'];
                    $height = $config['cf_member_icon_height'];
                    $icon_file_url = G5_DATA_URL.'/member_image/'.$mb_dir.'/'.get_mb_icon_name($mb_id).'.gif';
                    $name_tag['profile_image'] = '<span class="profile_img member_icon">';
                    $name_tag['profile_image'] .= '<img src="'.$icon_file_url.'" width="'.$width.'" height="'.$height.'" alt="'.$name.'">';
                    $name_tag['profile_image'] .= '</span>';
...
function get_member_level_icon($mb_level) {
    ...
    // 레벨별 기본 아이콘 파일명
    $icon_file = G5_DATA_PATH.'/member_level/level_'.$mb_level.'.png';
    
    // 파일이 존재하면 URL 반환
    if(file_exists($icon_file)) {
        return G5_DATA_URL.'/member_level/level_'.$mb_level.'.png';
    }
    
    // 레벨별 기본 아이콘이 없으면 빈 문자열 반환
    return '';
}

// 회원 레벨 아이콘 HTML 생성
function get_member_level_img($mb_id) {
    global $g5;
    ...
    $icon = get_member_level_icon($mb['mb_level']);
    
    if($icon) {
        $level_name = get_member_level_name($mb['mb_level']);
        return '<img src="'.$icon.'" alt="'.$level_name.'" title="'.$level_name.'" class="member_level_icon level_'.$mb['mb_level'].'">';
    }
...
function get_member_level_name($mb_level) {
    ...
    return isset($level_names[$mb_level]) ? $level_names[$mb_level] : '레벨'.$mb_level;
}

// 닉네임과 레벨 아이콘을 함께 출력
function get_text_with_member_level($name, $mb_id, $mb_level='') {
    global $g5, $config, $board;
    
    if(!$mb_id) return $name;
    
    // mb_level이 전달되지 않았으면 회원 정보에서 가져오기
    if(!$mb_level) {
        $mb = get_member($mb_id);
    ...
    // 레벨 아이콘 사용 여부 확인 (관리자에서 설정 가능하도록)
    $use_level_icon = true; // 기본값 true, 나중에 설정으로 변경 가능
    
    if($use_level_icon && $mb_level) {
        $icon_url = get_member_level_icon($mb_level);
        if($icon_url) {
            $level_name = get_member_level_name($mb_level);
            $level_icon = ' <img src="'.$icon_url.'" alt="'.$level_name.'" title="'.$level_name.'" class="member_level_icon level_'.$mb_level.'">';
        }
    }
    
    return $name . $level_icon;
}