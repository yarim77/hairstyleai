...
function showContent() {
    ...
    // 이렇게 하면 백버튼 눌렀을 때 갈 곳이 없어서 앱이 종료됨
    if (typeof history !== 'undefined') {
        // 히스토리 길이를 1로 만들어서 이전 페이지 제거
...
function closeSplash() {
    ...
    // ============================================
    // [기존 방식] window.location.href
    //   - 현재 페이지(splash.php)를 히스토리에 남기고 새 페이지로 이동
    //   - 결과: 백버튼 누르면 splash.php로 돌아옴 ❌
    //   - 히스토리: [start_app.php] → [splash.php] → [메인페이지]
    //
    // [수정 방식] window.location.replace
    //   - 현재 페이지(splash.php)를 히스토리에서 제거하고 새 페이지로 교체
    //   - 결과: 백버튼 눌러도 splash.php가 히스토리에 없어서 이전 페이지로 이동 ✅
    //   - 히스토리: [start_app.php] → [메인페이지] (splash.php 제거됨)
    //
    // 추가 방어: 위 세션 체크와 함께 사용하여 이중 방어
    //   - replace로 히스토리 제거 (1차 방어)
    //   - 만약 직접 URL 접근 시 세션 체크 (2차 방어)
    // ============================================
...
// ============================================
<?php if ($is_app) { ?>
// 앱에서는 백버튼 누르면 스플래시 건너뛰고 바로 메인으로 이동
// 또는 히스토리를 완전히 지워서 앱 종료되게 만듦
history.pushState(null, null, location.href);
window.onpopstate = function() {
    // 백버튼 눌렀을 때 메인으로 강제 이동
    window.location.replace('<?php echo $next_url; ?>');
};
<?php } else { ?>
// 웹에서는 기존 방식 유지 (뒤로가기 방지)
history.pushState(null, null, location.href);
window.onpopstate = function() {
    history.go(1);
};
