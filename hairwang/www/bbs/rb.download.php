...
@include_once($board_skin_path.'/rb.download.head.skin.php');
...
// 비회원은 다운로드 불가
if($is_guest) {
    alert('다운로드 권한이 없습니다.\\n회원이시라면 로그인 후 이용해 보십시오.', G5_BBS_URL.'/login.php?wr_id='.$w_id.'&amp;'.$qstr.'&amp;url='.urlencode(get_pretty_url($bo_table, $w_id, '#c_'.$wr_id)));
}
...
if ($member['mb_level'] < $board['bo_download_level']) {
    ...
    if ($member['mb_id'])
        ...
        alert($alert_msg.'\\n회원이시라면 로그인 후 이용해 보십시오.', G5_BBS_URL.'/login.php?wr_id='.$wr_id.'&amp;'.$qstr.'&amp;url='.urlencode(get_pretty_url($bo_table, $w_id, '#c_'.$wr_id)));
}
...
// 사용자 코드 실행
@include_once($board_skin_path.'/rb.download.skin.php');
...
@include_once($board_skin_path.'/rb.download.tail.skin.php');
